...
const ProductsComp = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`, { headers })
      ...
      .then((data) =>
        setProducts(data.map((p) => ({ ...p, id: p._id.toString() })))
      )
      .catch((err) => console.error(err));
  }, []);
  ...
  const handleSaveProduct = () => {
    ...
    if (!currentProduct.design_details)
      errors.design_details = "Please fill out this field";

    // Validate at least one image
    if (
      imageFields.length === 0 ||
      imageFields.every((field) => !field.file && !field.isOriginal)
    ) {
    ...
    // Set saving state to show loading indicator only after validation passes
    setIsSaving(true);
      ...
      .then((data) => {
        ...
        if (currentProduct.homepage) {
          ...
          fetch(`${API_URL}/api/homepage-products/delete/${data.product._id}`, {
            ...
            .catch((err) =>
              console.error("Error removing from homepage:", err)
            );
        }
    ...
    <div className="p-2">
      <div className="row mb-4 top-bar align-items-center">
        <div className="col-12 col-md-4 mb-3 mb-md-0">
          <button
            className="btn btn-secondary border w-100 w-md-auto"
            onClick={onBack}
          >
            BACK TO DASHBOARD
          </button>
            ...
            <div className="modal-content shadow-lg">
              ...
              <div className="modal-body">
                  ...
                  {imageFields.map((field, index) => (
                    ...
                    >
                      <input
                        type="file"
                        className={`form-control ${
                          validationErrors.images ? "input-error" : ""
                        }`}
                        disabled={!isEditable}
                        accept="image/*"
                        onChange={(e) => {
                ...
                <div className="field-container">
                  <textarea
                    className={`form-control ${
                      validationErrors.design_details ? "input-error" : ""
                    }`}
                    disabled={!isEditable}
                ...
                {currentProduct.sizes.map((size, index) => {
                  const hasError =
                    ...
                    validationErrors.sizes.includes(index);
                  return (
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (hasError) {
                              const newErrors = { ...validationErrors };
                              if (newErrors.sizes) {
                                newErrors.sizes = newErrors.sizes.filter(
                                  (i) => i !== index
                                );
                                if (newErrors.sizes.length === 0)
                  ...
                  <>
                    ...
                    <div className="field-container">
                      <input
                        type="number"
                        className={`form-control ${
                          validationErrors.stock ? "input-error" : ""
                        }`}
                        disabled={!isEditable}
